...
5. **Resolve** the ticket. Sign in as customer again — the portal opens the CSAT form and blocks catalog until you submit a score.

**Pass:** the customer ticket is visible on the desk with the same title, and the customer cannot use catalog until CSAT is submitted.
...
**WFM** (`/wfm`) — occupancy, roster, **shift hours** (`/wfm/shifts`, SPV), skills, on-call, forecast, **reviews**, **tukar shift**. Occupancy and forecast follow the **account** filter. Roster, skills, on-call, and reviews are **tenant-wide**. Default shifts: **Pagi** / **Siang** / **Malam** / **24 jam** — hours are editable per tenant. Login is **not** clock-in. After sign-in the banner shows today's shift; **Clock in** sets **Tersedia / Available**. **Sibuk / Busy** and **Istirahat / Break** stay clocked-in but are not auto-assigned. **Offline** is clock-out (also on logout / idle). Agents open **Roster saya** (own week only) and **Tukar shift** (request/accept; SPV approves). SPV/admin apply the week or upload CSV/Excel. Classroom: clock-in and read occupancy/forecast; do **not** rewrite the shared roster, skills, on-call, or the seeded staff review unless the tenant is isolated. Lead/SPV: **Penilaian** → scores 1–5 → **Ask AI** (advisory) → Submit. Agent: **Akui**. Auto-assign needs Redis + `npm run worker`.
...
3. As **`sari.l1@novacrm.app`**: confirm the today-shift banner, **Clock in**, change presence to **Sibuk** then back to **Tersedia**, open **Roster saya** (own row only). Optional: **Tukar shift** with `budi.l1@novacrm.app` if both have roster that week (trainer as SPV approves).
