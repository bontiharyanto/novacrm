...
- **WFM** — occupancy/forecast mengikuti filter account; roster/skills/on-call/penilaian **tenant-wide**. Shift standar Pagi / Siang / Malam / 24 jam; apply minggu atau unggah CSV/Excel. **Tukar shift** di `/wfm/swaps` (agen → rekan → SPV). Agen: banner shift + clock-in + **Roster saya**. Presence Tersedia/Sibuk/Istirahat/Offline; hanya Tersedia yang auto-assign. Dispatch di assignment group. Auto-assign butuh worker. Kelas bersama: jangan rewrite roster/penilaian seed kecuali tenant terisolasi.
