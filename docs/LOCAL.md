...
10. Automation: [Workflows](http://localhost:3000/workflows) → **New flow** → pick **Standard / Normal / Complex**. Canvas supports condition (Yes/No) nodes. Ticket create/status/comment, **machine alert**, and **inbound message** can run BullMQ actions (assign, email, WhatsApp, Telegram, status, asset). Check **Recent runs** on the flow.
    - Auto-create ticket: `POST /api/webhooks/whatsapp` (secret `local-whatsapp-secret`), `/api/webhooks/telegram`, `/api/webhooks/email`, `/api/webhooks/alerts` (Prometheus/Grafana JSON), `/api/webhooks/generic`. Repeat alerts within 24h update the same ticket. **NETMON** (production NMS) uses `POST /api/v1/t/{slug}/webhooks/alerts` with `x-webhook-secret` — see [NETMON.md](NETMON.md).
...
23. WFM: [http://localhost:3000/wfm](http://localhost:3000/wfm) — occupancy, roster, **shift hours** (`/wfm/shifts`), **tukar shift** (`/wfm/swaps`: agen → rekan → SPV), skills, on-call, forecast, reviews (`/wfm/reviews`). Coverage + clock-in export: Reports → Workforce. Banner + **Clock in** after login; agents see **Roster saya**. Demo: `admin@novacrm.app` apply Pagi, then `sari.l1@novacrm.app` clock-in. Dispatch policy lives on each assignment group.
