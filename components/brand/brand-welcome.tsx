...
import { WELCOME_COOKIE, WELCOME_COOKIE_LEGACY } from '@/lib/auth/welcome-cookie';

function consumeWelcomeFlag() {
  if (typeof window === 'undefined') return false;
  const params = new URLSearchParams(window.location.search);
  const fromQuery = params.get('welcome') === '1';
  const cookieNames = [WELCOME_COOKIE, WELCOME_COOKIE_LEGACY];
  const fromCookie = document.cookie.split(';').some((part) => {
    const value = part.trim();
    return cookieNames.some((name) => value.startsWith(`${name}=`));
  });
...
export function BrandWelcome({ variant }: { variant: 'portal' | 'desk' }) {
  const { t } = useI18n();
        ...
        >
          ...
          >
            ...
            <p className="mt-1 font-mono text-[11px] uppercase tracking-[0.22em] text-zinc-500">
              {variant === 'portal' ? t.brand.portal : t.brand.operations}
            </p>
          </motion.div>
        </motion.div>
      ) : null}
