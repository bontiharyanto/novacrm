...
import { listUcCredits } from '@/lib/uc/credits';
...
export default async function UcDetailPage({ params }: { params: { id: string } }) {
  ...
  const [contract, credits] = await Promise.all([
    getUnderpinningContract(params.id),
    listUcCredits(params.id),
  ]);
  if (!contract) notFound();
  return <UcEditor contract={contract} credits={credits} canEdit={canRole(session.profile.role, 'update', 'Sla')} />;
}
